...
export default function Module(props) {
  ...
  return (
    <div 
      ...
      className="bg-white hover:bg-slate-50  max-w-5xl mx-auto py-2  rounded-lg shadow-xl my-3"
    >
      <div className="flex flex-row  mb-4 justify-between">
        <a 
          onClick={() => { navigate("/moduleEdit") }}
          className="text-2xl ml-6 text-gray-700 font-bold hover:underline hover:cursor-pointer"
        >
          {props.obj.nombre}
        </a>
      ...
      <div className="flex items-center justify-between mb-4 mx-6">
        <p className="text-gray-500 uppercase text-sm">{props.obj.fecha}</p>
        <span className="bgColorCustom text-white py-2 px-4 text-xs rounded uppercase">
          {props.obj.nivel}
        </span>
      </div>
      {modalEdit && <ModalCreateEdit obj={props.obj} onClose={CloseModalEdit} visible={modalEdit} handleValue={handleObj} type={"modulo"} />}
    </div>
  )
}
